...
<div class="row">
	<div class="col-sm-2 bar_navegacion">
		<a href="{{route('admin.courses.edit', [$theme->session->course->id, 'target' => urlencode('#sesion-tab')])}}" class="btn btn-return">Regresar a sesiones</a>
		<h5>Sesion: {{$theme->session->name}}</h5>
		@foreach($theme->session->themes as $theme_item)
		<a href="{{route('admin.themes.edit', $theme_item->id)}}" class="btn btn-tema" title="{{$theme_item->name}}">Tema {{$loop->iteration}}</a>
		@endforeach
	</div>

	<div class="col-sm-10 container_crear_tema">
			...
			<div class="tab-pane fade show active" id="clase" role="tabpanel" aria-labelledby="clase-tab">
				<div class="row">
					<div class="col-sm-6 col-md-6">
						<form method="post" enctype="multipart/form-data" action="{{route('admin.themes.add-multimedia', $theme->id)}}">
						{!! csrf_field() !!}
								...
								<div class="col-sm-12 col-md-12 ">
									<div  class="row upload_text_material">
										...
										<div class="col-sm-3 col-md-2 align-self-center save_button_material">
											...
											<a href="{{route('admin.themes.delete-multimedia', [$theme->id, $file->id])}}" class="ml-1 align-self-center text-danger" title="Eliminar clase"><span class="fas fa-trash-alt"></span></a>
										</div>				
									</div>
								</div>
							</div>
							@endforeach
							<a class="btn btn-secondary mt-2 mb-3" href="{{route('admin.themes.student-view', $theme->id)}}" title="Ver como alumno">Ver como alumno</a>
						</div>
				...
				<div class="main_container_material">
					<div class="container_list_material py-2">
						...
						<div class="row upload_material">
								<div class="col-sm-12 col-md-12 ">
									<form method="post" action="{{route('admin.themes.add-attachment', $theme->id)}}" enctype="multipart/form-data">
									{!! csrf_field() !!}
									<div  class="row upload_text_material">
							...
							<div class="col-sm-12 col-md-12 ">
								<div  class="row upload_text_material">
									...
									<div class="col-sm-6 col-md-6 save_button_material">
										...
										<a href="{{route('admin.themes.delete-attachment', [$theme->id, $attachment->id])}}" class="text-danger" title="Eliminar material"><span class="fas fa-trash-alt"></span></a>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="col-sm-12 col-md-12 ">
								<div  class="row upload_text_ejercicio">
									...
									<div class="col-sm-6 col-md-2 align-self-center save_button_ejercicio">
										<a id="add_exercise_link" href="{{route('admin.themes.add-exercise', request()->route('id'))}}" class="btn btn-secondary">Añadir Preguntas</a>
									</div>				
								</div>
							</div>
						</div>
							...
							<div class="col-sm-12 col-md-12 ">
								<div  class="row upload_text_ejercicio">
									...
									<div class="col-sm-6 col-md-6 options_ejercicio">
										<a href="{{route('admin.questions.create', ['entity_name' => ENTITY_EXERCISE, 'entity_id' => $exercise->id])}}" class="text-primary" title="Ver ejercicios"><span class="fas fa-search"></span></a>
										<a href="{{route('admin.themes.delete-exercise', [$theme->id, $exercise->id])}}" class="text-danger" title="Eliminar ejercicios"><span class="fas fa-trash-alt"></span></a>
									</div>				
								</div>
							</div>
						</div>
...
$('#description_file, #nombre_tema').keyup(function(){
    delay(function(){
        ...
        $.ajax({
            url: "{{route('admin.themes.edit', request()->route('id'))}}",
            type: "POST",
            data: {
                "name" : name,
