...
<div class="container container_main">
    <div class="container_added_preguntas">
        ...
        <form method="post" action="{{route('teacher.questions.review')}}" id="questions_form" enctype="multipart/form-data">
            {!! csrf_field() !!}
                    ...
                    @if($answer->first()->question->type_id == TYPE_SINGLE_RESPONSE)
                        ...
                        <div class="row">
                            <div class="col-md-9">
                                ...
                                <input type="radio" value="{{$option->value}}" {{$answer->first()->value == $option->value ? 'checked' : ''}} readonly disabled>{{$option->value}}<br>
                                @endforeach
                            </div>
                    ...
                    @elseif($answer->first()->question->type_id == TYPE_OPEN_RESPONSE)
                        ...
                        <div class="row">
                            <div class="col-md-9">  
                                ...
                                <textarea name="answer[{{$answer->first()->id}}][obs]">{{$answer->first()->observation or ''}}</textarea>
                            </div>
                            <div class="col-md-3 text_score">
                                ...
                                Puntaje otorgado:<span><input class="assigned_score" type="text" name="answer[{{$answer->first()->id}}][score]" value="{{$answer->first()->final_score or 0}}"> puntos</span>
                            </div>
                        </div>
...
$('.assigned_score').keyup(function(){
    var total = calculate_total_qualification();

    if ($(this).val() > $(this).parent().siblings('span').data('score')) {
        $(this).val(0);
        calculate_total_qualification();
    }

    if (total > parseFloat({{$questions->sum('pivot.score')}})) {
        $(this).val(0);
        calculate_total_qualification();
    }
});
